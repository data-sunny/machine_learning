...
```{r, accuracy estimation, echo=TRUE}
                set.seed(1234)
            ...
            # try "gbm" - 96 % on train data, 95 % on test data ( 22 min run time )
                # m3 <- train( classe ~., method="gbm", data=train_train )
                # confusionMatrix( train_train$classe, predict(m3, train_train))
                # confusionMatrix( train_test$classe, predict(m3, train_test))
            ...
            cat("Model GBM Accuracy on train data= 0.96", "& Accuracy on test data= 0.95 ")
...
```{r, cross validation, echo=TRUE}
            ...
            # try "gbm" on full set - 96 % on train data, 95 % on test data ( 22 min run time )
            ...
            # try "gbm" on full set - using train control
             ...
             # m34 <- train( classe ~., method="gbm", data=train_selects, trControl=train_control)
             # confusionMatrix( train_selects$classe, predict(m34, train_selects))
             
    
        cat(" Model GBM with repeated k-fold cross validation on entire training data set gives Accuracies")
        cat(" 0.72, 0.84, 0.88, 0.80, 0.90, 0.93, 0.84, 0.92, 0.95")
...
```{r, model application to test case, echo=TRUE}
...
```{r, generate output file, echo=TRUE }
